...
2. EventBridge is critical when connecting two different Codebuild or Codepipeline , stages or pipelines, as every step of a CodePipeline stage or a Codebuild success/failure results in an Event Bus event which can be used to trigger alternative pathways for example: In case of a failure triggering a lambda function to delete data deployed on S3 buckets before rolling back the deployment using Codebuild/Codepipeline. 

3. CodeDeploy needs CodeDeploy agent to be installed in target EC2 instances unlike lambda functions, API Gateway and ECS clusters. 
